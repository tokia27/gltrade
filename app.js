...
let pendingGoogleUser = null; // დროებით ინახავს Google-ით შესული მომხმარებლის მონაცემებს
// მიმდინარე ენის შენახვა (ნაგულისხმევად ქართული 'ka')
let currentLang = localStorage.getItem('site_lang') || 'ka';

// ენის შეცვლის ფუნქცია (როდესაც მომხმარებელი დააჭერს GE / EN / RU ღილაკებს)
function changeLanguage(lang) {
    currentLang = lang;
    localStorage.setItem('site_lang', lang);

    // ხელახლა ვრენდერებთ პროდუქტებს ახალ ენაზე
    const prodList = (typeof products !== 'undefined' ? products : []);
    renderProducts(prodList);
}

// LocalStorage-დან მომხმარებლების წამოღების დამხმარე ფუნქცია
...
// JWT ტოკენის დეკოდირების დამხმარე ფუნქცია
...
// Google Sign-In-ის callback ფუნქცია
function handleCredentialResponse(response) {
    ...
    if (foundUser) {
        ...
        // თუ ახალი მომხმარებელია Google-იდან, ვითხოვთ ტელეფონს და მისამართს
        pendingGoogleUser = {
            username: name,
            email: email,
            googleId: googleId,
...
// Google-ით დარეგისტრირებული მომხმარებლის დამატებითი ინფოს შენახვა
        ...
        return `
            <div class="product-card ${!isAvailable ? 'out-of-stock-card' : ''}" id="card-${prod.id}" style="position: relative;">
                ...
                <div class="product-img-container" style="height: 200px; border-radius: 8px; overflow: hidden; position: relative; opacity: ${isAvailable ? '1' : '0.5'};">
                    <img id="card-img-${prod.id}" src="${imgList[currentImageIndexes[prod.id]]}" class="product-img" alt="${productName}" onclick="openProductModal(${prod.id})" style="width:100%; height:100%; object-fit:cover; cursor:pointer;">
...
function openProductModal(productId) {
    ...
    // 🌐 1. დასახელების ამოღება ენის მიხედვით
    ...
    // 🌐 2. აღწერის ამოღება ენის მიხედვით
    let descText = '';
    ...
    modalBody.innerHTML = `
        <div class="modal-product-details">
            <div>
                <img id="modal-img-${prod.id}" src="${imgList[0]}" style="width:100%; border-radius:8px; max-height:300px; object-fit:cover;">
            </div>
            <div>
                <h2>${productName}</h2>
...
async function checkout() {
    ...
    // =========================================================
    // 2.1. მინიმალური თანხის ვალიდაცია (ქაღალდი vs სხვა პროდუქცია)
    // =========================================================
    let paperTotal = 0;
    let otherTotal = 0;
    ...
    cart.forEach(item => {
        ...
        // ვამოწმებთ არის თუ არა პროდუქტი საბეჭდი ქაღალდი
        // (ამოწმებს category, isPaper დროშას ან დასახელებას)
    ...
    // ა) შემთხვევა 1: კალათაში მხოლოდ საბეჭდი ქაღალდია
    ...
    // ბ) შემთხვევა 2: კალათაში არის სხვა საკანცელარიო/ოფისის პროდუქტებიც (ან მხოლოდ ისინი)
    if (otherTotal > 0) {
        if (otherTotal < 50) {
            alert(`⚠️ შეკვეთის გასაფორმებლად პროდუქტების ჯამური თანხა (საბეჭდი ქაღალდის გარეშე) უნდა იყოს მცირე 50 ₾-ზე.\nთქვენი პროდუქტების ჯამი (ქაღალდის გარეშე): ${otherTotal.toFixed(2)} ₾`);
            return;
        }
    }
    // =========================================================

    // 3. ღილაკის გათიშვა (Loading სტატუსი ორჯერ დაჭერის თავიდან ასაცილებლად)
    ...
    const userInfo = {
        ...
        phone: currentUser.phone || 'არ არის მითითებული'
    };
    ...
    try {
        // 5. ავტორიზაციის ტოკენის წამოღება (თუ გამოიყენება)
        ...
        if (response.ok) {
            alert(`🎉 შეკვეთა ${orderId} წარმატებით გაფორმდა!`);

            // 7. მომხმარებლის ისტორიის განახლება
            if (!currentUser.orderHistory) {
                currentUser.orderHistory = [];
            }
            currentUser.orderHistory.unshift(orderData);
            
            // LocalStorage-ის განახლება
            localStorage.setItem('logged_user', JSON.stringify(currentUser));
            
            if (typeof getUsersFromStorage === 'function') {
                const users = getUsersFromStorage();
                const userIndex = users.findIndex(u => u.username === currentUser.username || u.id === currentUser.id);
                if (userIndex !== -1) {
            ...
            // 8. კალათის გასუფთავება და გვერდის განახლება
            cart = [];
            localStorage.removeItem('cart'); // თუ კალათას LocalStorage-ში ინახავთ
            ...
            alert(`⚠️ შეკვეთის გაგზავნა ვერ მოხერხდა: ${result.message || 'შეცდომა სერვერზე'}`);
        }

    } catch (err) {
        console.error('სერვერთან კავშირი ვერ დამყარდა:', err);
        alert('❌ სერვერთან კავშირი ვერ დამყარდა! დარწმუნდით, რომ სერვერი ჩართულია.');
    } finally {
        // 9. ღილაკის პირვანდელ მდგომარეობაში დაბრუნება
        if (checkoutBtn) {
